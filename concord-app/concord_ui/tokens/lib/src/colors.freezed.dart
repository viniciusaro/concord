// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target

part of 'colors.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more informations: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

/// @nodoc
class _$ConcordColorsTearOff {
  const _$ConcordColorsTearOff();

  _ConcordColors call(
      {required Color primary,
      required Color primaryText,
      required Color primaryAction,
      required Color primaryBackground,
      required Color secondary,
      required Color secondaryText,
      required Color secondaryAction,
      required Color secondaryBackground,
      required Color tertiary,
      required Color tertiaryText,
      required Color tertiaryAction,
      required Color tertiaryBackground}) {
    return _ConcordColors(
      primary: primary,
      primaryText: primaryText,
      primaryAction: primaryAction,
      primaryBackground: primaryBackground,
      secondary: secondary,
      secondaryText: secondaryText,
      secondaryAction: secondaryAction,
      secondaryBackground: secondaryBackground,
      tertiary: tertiary,
      tertiaryText: tertiaryText,
      tertiaryAction: tertiaryAction,
      tertiaryBackground: tertiaryBackground,
    );
  }
}

/// @nodoc
const $ConcordColors = _$ConcordColorsTearOff();

/// @nodoc
mixin _$ConcordColors {
  Color get primary => throw _privateConstructorUsedError;
  Color get primaryText => throw _privateConstructorUsedError;
  Color get primaryAction => throw _privateConstructorUsedError;
  Color get primaryBackground => throw _privateConstructorUsedError;
  Color get secondary => throw _privateConstructorUsedError;
  Color get secondaryText => throw _privateConstructorUsedError;
  Color get secondaryAction => throw _privateConstructorUsedError;
  Color get secondaryBackground => throw _privateConstructorUsedError;
  Color get tertiary => throw _privateConstructorUsedError;
  Color get tertiaryText => throw _privateConstructorUsedError;
  Color get tertiaryAction => throw _privateConstructorUsedError;
  Color get tertiaryBackground => throw _privateConstructorUsedError;

  @JsonKey(ignore: true)
  $ConcordColorsCopyWith<ConcordColors> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $ConcordColorsCopyWith<$Res> {
  factory $ConcordColorsCopyWith(
          ConcordColors value, $Res Function(ConcordColors) then) =
      _$ConcordColorsCopyWithImpl<$Res>;
  $Res call(
      {Color primary,
      Color primaryText,
      Color primaryAction,
      Color primaryBackground,
      Color secondary,
      Color secondaryText,
      Color secondaryAction,
      Color secondaryBackground,
      Color tertiary,
      Color tertiaryText,
      Color tertiaryAction,
      Color tertiaryBackground});
}

/// @nodoc
class _$ConcordColorsCopyWithImpl<$Res>
    implements $ConcordColorsCopyWith<$Res> {
  _$ConcordColorsCopyWithImpl(this._value, this._then);

  final ConcordColors _value;
  // ignore: unused_field
  final $Res Function(ConcordColors) _then;

  @override
  $Res call({
    Object? primary = freezed,
    Object? primaryText = freezed,
    Object? primaryAction = freezed,
    Object? primaryBackground = freezed,
    Object? secondary = freezed,
    Object? secondaryText = freezed,
    Object? secondaryAction = freezed,
    Object? secondaryBackground = freezed,
    Object? tertiary = freezed,
    Object? tertiaryText = freezed,
    Object? tertiaryAction = freezed,
    Object? tertiaryBackground = freezed,
  }) {
    return _then(_value.copyWith(
      primary: primary == freezed
          ? _value.primary
          : primary // ignore: cast_nullable_to_non_nullable
              as Color,
      primaryText: primaryText == freezed
          ? _value.primaryText
          : primaryText // ignore: cast_nullable_to_non_nullable
              as Color,
      primaryAction: primaryAction == freezed
          ? _value.primaryAction
          : primaryAction // ignore: cast_nullable_to_non_nullable
              as Color,
      primaryBackground: primaryBackground == freezed
          ? _value.primaryBackground
          : primaryBackground // ignore: cast_nullable_to_non_nullable
              as Color,
      secondary: secondary == freezed
          ? _value.secondary
          : secondary // ignore: cast_nullable_to_non_nullable
              as Color,
      secondaryText: secondaryText == freezed
          ? _value.secondaryText
          : secondaryText // ignore: cast_nullable_to_non_nullable
              as Color,
      secondaryAction: secondaryAction == freezed
          ? _value.secondaryAction
          : secondaryAction // ignore: cast_nullable_to_non_nullable
              as Color,
      secondaryBackground: secondaryBackground == freezed
          ? _value.secondaryBackground
          : secondaryBackground // ignore: cast_nullable_to_non_nullable
              as Color,
      tertiary: tertiary == freezed
          ? _value.tertiary
          : tertiary // ignore: cast_nullable_to_non_nullable
              as Color,
      tertiaryText: tertiaryText == freezed
          ? _value.tertiaryText
          : tertiaryText // ignore: cast_nullable_to_non_nullable
              as Color,
      tertiaryAction: tertiaryAction == freezed
          ? _value.tertiaryAction
          : tertiaryAction // ignore: cast_nullable_to_non_nullable
              as Color,
      tertiaryBackground: tertiaryBackground == freezed
          ? _value.tertiaryBackground
          : tertiaryBackground // ignore: cast_nullable_to_non_nullable
              as Color,
    ));
  }
}

/// @nodoc
abstract class _$ConcordColorsCopyWith<$Res>
    implements $ConcordColorsCopyWith<$Res> {
  factory _$ConcordColorsCopyWith(
          _ConcordColors value, $Res Function(_ConcordColors) then) =
      __$ConcordColorsCopyWithImpl<$Res>;
  @override
  $Res call(
      {Color primary,
      Color primaryText,
      Color primaryAction,
      Color primaryBackground,
      Color secondary,
      Color secondaryText,
      Color secondaryAction,
      Color secondaryBackground,
      Color tertiary,
      Color tertiaryText,
      Color tertiaryAction,
      Color tertiaryBackground});
}

/// @nodoc
class __$ConcordColorsCopyWithImpl<$Res>
    extends _$ConcordColorsCopyWithImpl<$Res>
    implements _$ConcordColorsCopyWith<$Res> {
  __$ConcordColorsCopyWithImpl(
      _ConcordColors _value, $Res Function(_ConcordColors) _then)
      : super(_value, (v) => _then(v as _ConcordColors));

  @override
  _ConcordColors get _value => super._value as _ConcordColors;

  @override
  $Res call({
    Object? primary = freezed,
    Object? primaryText = freezed,
    Object? primaryAction = freezed,
    Object? primaryBackground = freezed,
    Object? secondary = freezed,
    Object? secondaryText = freezed,
    Object? secondaryAction = freezed,
    Object? secondaryBackground = freezed,
    Object? tertiary = freezed,
    Object? tertiaryText = freezed,
    Object? tertiaryAction = freezed,
    Object? tertiaryBackground = freezed,
  }) {
    return _then(_ConcordColors(
      primary: primary == freezed
          ? _value.primary
          : primary // ignore: cast_nullable_to_non_nullable
              as Color,
      primaryText: primaryText == freezed
          ? _value.primaryText
          : primaryText // ignore: cast_nullable_to_non_nullable
              as Color,
      primaryAction: primaryAction == freezed
          ? _value.primaryAction
          : primaryAction // ignore: cast_nullable_to_non_nullable
              as Color,
      primaryBackground: primaryBackground == freezed
          ? _value.primaryBackground
          : primaryBackground // ignore: cast_nullable_to_non_nullable
              as Color,
      secondary: secondary == freezed
          ? _value.secondary
          : secondary // ignore: cast_nullable_to_non_nullable
              as Color,
      secondaryText: secondaryText == freezed
          ? _value.secondaryText
          : secondaryText // ignore: cast_nullable_to_non_nullable
              as Color,
      secondaryAction: secondaryAction == freezed
          ? _value.secondaryAction
          : secondaryAction // ignore: cast_nullable_to_non_nullable
              as Color,
      secondaryBackground: secondaryBackground == freezed
          ? _value.secondaryBackground
          : secondaryBackground // ignore: cast_nullable_to_non_nullable
              as Color,
      tertiary: tertiary == freezed
          ? _value.tertiary
          : tertiary // ignore: cast_nullable_to_non_nullable
              as Color,
      tertiaryText: tertiaryText == freezed
          ? _value.tertiaryText
          : tertiaryText // ignore: cast_nullable_to_non_nullable
              as Color,
      tertiaryAction: tertiaryAction == freezed
          ? _value.tertiaryAction
          : tertiaryAction // ignore: cast_nullable_to_non_nullable
              as Color,
      tertiaryBackground: tertiaryBackground == freezed
          ? _value.tertiaryBackground
          : tertiaryBackground // ignore: cast_nullable_to_non_nullable
              as Color,
    ));
  }
}

/// @nodoc

class _$_ConcordColors implements _ConcordColors {
  _$_ConcordColors(
      {required this.primary,
      required this.primaryText,
      required this.primaryAction,
      required this.primaryBackground,
      required this.secondary,
      required this.secondaryText,
      required this.secondaryAction,
      required this.secondaryBackground,
      required this.tertiary,
      required this.tertiaryText,
      required this.tertiaryAction,
      required this.tertiaryBackground});

  @override
  final Color primary;
  @override
  final Color primaryText;
  @override
  final Color primaryAction;
  @override
  final Color primaryBackground;
  @override
  final Color secondary;
  @override
  final Color secondaryText;
  @override
  final Color secondaryAction;
  @override
  final Color secondaryBackground;
  @override
  final Color tertiary;
  @override
  final Color tertiaryText;
  @override
  final Color tertiaryAction;
  @override
  final Color tertiaryBackground;

  @override
  String toString() {
    return 'ConcordColors(primary: $primary, primaryText: $primaryText, primaryAction: $primaryAction, primaryBackground: $primaryBackground, secondary: $secondary, secondaryText: $secondaryText, secondaryAction: $secondaryAction, secondaryBackground: $secondaryBackground, tertiary: $tertiary, tertiaryText: $tertiaryText, tertiaryAction: $tertiaryAction, tertiaryBackground: $tertiaryBackground)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _ConcordColors &&
            const DeepCollectionEquality().equals(other.primary, primary) &&
            const DeepCollectionEquality()
                .equals(other.primaryText, primaryText) &&
            const DeepCollectionEquality()
                .equals(other.primaryAction, primaryAction) &&
            const DeepCollectionEquality()
                .equals(other.primaryBackground, primaryBackground) &&
            const DeepCollectionEquality().equals(other.secondary, secondary) &&
            const DeepCollectionEquality()
                .equals(other.secondaryText, secondaryText) &&
            const DeepCollectionEquality()
                .equals(other.secondaryAction, secondaryAction) &&
            const DeepCollectionEquality()
                .equals(other.secondaryBackground, secondaryBackground) &&
            const DeepCollectionEquality().equals(other.tertiary, tertiary) &&
            const DeepCollectionEquality()
                .equals(other.tertiaryText, tertiaryText) &&
            const DeepCollectionEquality()
                .equals(other.tertiaryAction, tertiaryAction) &&
            const DeepCollectionEquality()
                .equals(other.tertiaryBackground, tertiaryBackground));
  }

  @override
  int get hashCode => Object.hash(
      runtimeType,
      const DeepCollectionEquality().hash(primary),
      const DeepCollectionEquality().hash(primaryText),
      const DeepCollectionEquality().hash(primaryAction),
      const DeepCollectionEquality().hash(primaryBackground),
      const DeepCollectionEquality().hash(secondary),
      const DeepCollectionEquality().hash(secondaryText),
      const DeepCollectionEquality().hash(secondaryAction),
      const DeepCollectionEquality().hash(secondaryBackground),
      const DeepCollectionEquality().hash(tertiary),
      const DeepCollectionEquality().hash(tertiaryText),
      const DeepCollectionEquality().hash(tertiaryAction),
      const DeepCollectionEquality().hash(tertiaryBackground));

  @JsonKey(ignore: true)
  @override
  _$ConcordColorsCopyWith<_ConcordColors> get copyWith =>
      __$ConcordColorsCopyWithImpl<_ConcordColors>(this, _$identity);
}

abstract class _ConcordColors implements ConcordColors {
  factory _ConcordColors(
      {required Color primary,
      required Color primaryText,
      required Color primaryAction,
      required Color primaryBackground,
      required Color secondary,
      required Color secondaryText,
      required Color secondaryAction,
      required Color secondaryBackground,
      required Color tertiary,
      required Color tertiaryText,
      required Color tertiaryAction,
      required Color tertiaryBackground}) = _$_ConcordColors;

  @override
  Color get primary;
  @override
  Color get primaryText;
  @override
  Color get primaryAction;
  @override
  Color get primaryBackground;
  @override
  Color get secondary;
  @override
  Color get secondaryText;
  @override
  Color get secondaryAction;
  @override
  Color get secondaryBackground;
  @override
  Color get tertiary;
  @override
  Color get tertiaryText;
  @override
  Color get tertiaryAction;
  @override
  Color get tertiaryBackground;
  @override
  @JsonKey(ignore: true)
  _$ConcordColorsCopyWith<_ConcordColors> get copyWith =>
      throw _privateConstructorUsedError;
}
